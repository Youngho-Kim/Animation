    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btnProp:
                ...
                break;
            case R.id.btnRed:
                ObjectAnimator transBackY = ObjectAnimator.ofFloat(
                        btnRed,         // 움직일 대상
                        "translationY", //애니메이션 속성
                        0             // 속성 값
                );
                ObjectAnimator transBackX = ObjectAnimator.ofFloat(
                        btnRed,         // 움직일 대상
                        "translationX", //애니메이션 속성
                        0             // 속성 값
                );
                ...
                // 애니메이터 셋을 구성해서 실행한다.
                AnimatorSet aniSetBack = new AnimatorSet();
                aniSetBack.playTogether(transBackX, transBackY, rotateBack); // 개수의 제한이 없슴
                aniSetBack.setDuration(3000);           // 애니메이터 셋의 실행 시간
                // 처음엔 빨리 점점 느리게
                aniSetBack.setInterpolator(new AccelerateDecelerateInterpolator());
                aniSetBack.start();
                break;
        }
    }
